...
{
    public class PimpedRacerAI : BasicNavigationalAI
    {
        ...
        public override void boot(Structure structure, HelmController helm = null)
        {
            ...
            BetterAvoidBehaviour avoidBehaviour = Helm.getBehaviourByName("AVOID") as BetterAvoidBehaviour;
            ...
            nextRaceGate = navpointsAndIDs[1].navPoint.GetComponent<RaceGate>();
            nextRaceGate.NotifyRacer += enteredGate;
        }
        ...
        public void enteredGate(object sender, RacerEventArgs args)
        {
            if (args.structure != structure) return;

            Gui.setMessage(structure.Name + " has hit gate " + args.curGate.gameObject.name);

            // unsubscribe from current race gate entered event
            if (args.curGate != null)
            {
                args.curGate.NotifyRacer -= enteredGate;
            }

            args.nextGate.NotifyRacer += enteredGate;

            nextRaceGate = args.nextGate;

            string navpointName = nextRaceGate.gameObject.name;

            if (navpointName == "NavPoint07" || navpointName == "NavPoint17")
            {
