...
namespace Davin.Controllers
{
    ...
    public class MyBestSquadronAI : AIStateController, IFormationFly, ILand
    {
        ...
        public virtual string combatAction()
        {
            if (structure.scanner.isActiveOn() == true)
            {
                ...
                if (enemies.Count > 0)
                {
                    ...
                    if (primaryTargetSystem != null && primaryTargetSystem.destroyed)
                    {
                        // switch system
                        List<IBlasterTurret> blasters = primaryTargetStructure.getWeapons<IBlasterTurret>(); 
                        ...
                        foreach (IBlasterTurret blaster in blasters)
                        {
                            ...
                            primaryTargetSystem = blaster as Module;
                            break;
                        }

                        if (primaryTargetSystem.destroyed == true)
                        {
                            List<ILaserTurret> lasers = primaryTargetStructure.getWeapons<ILaserTurret>();
                            ...
                            foreach (ILaserTurret laser in lasers)
                            {
                                ...
                                primaryTargetSystem = laser as Module;
                                break;
                            }
                        }
                    }
